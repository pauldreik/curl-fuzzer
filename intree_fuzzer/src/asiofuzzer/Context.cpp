...
void
Context::onFastTimer(boost::system::error_code ec)
{
  if(!ec) {
    if(debugoutput)
...
Context::Context(IOCONTEXT &io) : m_io(io), m_fast_timer(io)
{
...
Context::setoptions(const uint8_t *data, size_t size)
{
  ...
  auto selector = [&](long defaultchoice,
  ...
  auto setoption = [&](CURLoption curloption, auto value) {
    int ret = curl_easy_setopt(m_easy, curloption, value);
    assert(ret == 0);
  };
  ...
  ret =
    curl_easy_setopt(context.m_easy, CURLOPT_READFUNCTION, fuzz_read_callback);
  assert(ret == 0);
  ...
  if(usedoh) {
    ret = curl_easy_setopt(
      context.m_easy, CURLOPT_DOH_URL, "http://127.0.0.1/doh");
    assert(ret == 0);
  }

  const bool useproxy = false;
  if(useproxy) {
    ret = curl_easy_setopt(context.m_easy, CURLOPT_PROXY, "http://127.0.0.1/");
    assert(ret == 0);
...
void
Context::splitFuzzDataIntoResponses(const uint8_t *data, size_t size)
{
